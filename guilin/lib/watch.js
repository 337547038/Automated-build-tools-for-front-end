...
module.exports = function (type) {
    ...
    chokidar.watch('src', {ignored: /(^|[\/\\])\../}).on('all', function (event, path) {
        ...
        switch (event) {
            case 'addDir':
                //这里只创建目录即可，除开两个特殊的sass,model
                if (path.indexOf('src/sass') != -1 || path.indexOf('src/model') != -1) {
                } else {
            ...
            case 'change':
                ...
                if (suffix.indexOf(fileExtension) != -1) {
...
var editMainJs = function (path) {
    ...
    fs.writeFile("./main.js", content, function (err) {
        if (err) throw err;
    });
};